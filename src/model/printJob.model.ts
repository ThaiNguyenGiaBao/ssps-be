...
class PrintingJobModel {
    ...
    static async getPrintJobByUserAndPrinter({userId, printerId, startDate, endDate}: {userId: string, printerId: string, startDate: string, endDate: string}) {
        ...
        if(printerId == 'none' && userId == 'none')
            allPrintJob = await db.query("SELECT * FROM printingjob WHERE starttime >= '"+ startDate +"' AND starttime <= '"+ endDate +"'");
        else if(printerId == 'none')
            allPrintJob = await db.query("SELECT * FROM printingjob WHERE userid = $1 AND starttime >= '"+ startDate +"' AND starttime <= '"+ endDate +"'", [userId]);
        else if(userId == 'none')
            allPrintJob = await db.query("SELECT * FROM printingjob WHERE printerid = $1 AND starttime >=  '"+ startDate +"' AND starttime <= '"+ endDate +"'", [printerId]);
        else
            allPrintJob = await db.query("SELECT * FROM printingjob WHERE userid = $1 AND printerid = $2 AND starttime >= '"+ startDate +"' AND starttime <= '"+ endDate +"'", 
                          [userId, printerId]);
