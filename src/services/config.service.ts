...
export class ConfigService {
    ...
    static async updatePermitedFile(type: string, data: Partial<PermitedFile>) {
        if (!type) throw new BadRequestError("File type is required.");
        ...
        if (result === null) throw new BadRequestError("Cannot update this permited file config.");
        return result;
    }
    ...
    static async deletePermitedFile(type: string): Promise<PermitedFile> {
        ...
        if (result === null) throw new NotFoundError("Cannot find that permited file type!");
        return result;
    }
    ...
    static async updatePageConfig(data: Partial<Config>): Promise<Config> {
        ...
        if (data.dategivenpage && isNaN(Date.parse(data.dategivenpage))) throw new BadRequestError("Not valid date format!");

        const result = await ConfigModel.updatePageConfig(data);
        if (result === null) throw new BadRequestError("Update unsuccessfully");
        return result;
    }
}
